...
namespace MindWaveReaderWPF
{
    ...
    public partial class WindowMain : Window
    {
        ...
        bool demoVariable = false;
        int count = 15;
        /*private void ButtonDemo_Click(object sender, RoutedEventArgs e)
        {
            int temp = 0;
            // to turn on Demo versiyon
            //if bool 0 it will run it is 1 it will stop

            if (demoVariable == false)
            {
                demoVariable = true;
                while (temp<count)
                {
        ...
        */
        ...
        private void TGMockDataGeneratorAttention()
        {
            var TGDataRow = new TGData();
            var Rand = new Random((int)DateTime.Now.Ticks);

            // Generate Randome Data 30%
            if (Rand.Next(1, 101) < 30) TGDataRow.PoorSignal = Rand.Next(0, 45);
            if (Rand.Next(1, 101) < 30) TGDataRow.Attention = Rand.Next(50, 75);
            if (Rand.Next(1, 101) < 30) TGDataRow.Meditation = Rand.Next(0, 20);
        ...
        private void TGMockDataGeneratorCalm()
        {
            var TGDataRow = new TGData();
            var Rand = new Random((int)DateTime.Now.Ticks);

            // Generate Randome Data 30%
            if (Rand.Next(1, 101) < 30) TGDataRow.PoorSignal = Rand.Next(15, 45);
            if (Rand.Next(1, 101) < 30) TGDataRow.Attention = Rand.Next(15, 30);
            if (Rand.Next(1, 101) < 30) TGDataRow.Meditation = Rand.Next(60, 80);
